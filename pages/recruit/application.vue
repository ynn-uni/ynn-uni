  ...
  <view class="container">
    ...
    <view class="application">
      ...
      >
          ...
          <view class="formgroup">
            <bjx-form-item labelType="block" label="4.患者的性别：" prop="sex">
              <radio-group class="block" @change="RadioChange" name="sex" v-model="form.sex">
                <radio
                  class="radio ra"
                  :class="form.sex==1?'checked':''"
                  :checked="form.sex==1?true:false"
                  :value="1"
                ></radio>男
                <radio
                  class="radio ra"
                  :class="form.sex==2?'checked':''"
                  :checked="form.sex==2?true:false"
                  :value="2"
                ></radio>女
              </radio-group>
            </bjx-form-item>
        ...
        <view class="note text-center margin-top-xs">注：请携带相关资料，比如病例、检测报告等资料</view>
        <view class="treaty flex justify-center align-center">
          <label>
            <checkbox value="cb" checked="true" color="#FFFFFF" style="transform:scale(0.7)" />
            <text @click.stop="treaty">我已阅读，并同意接受相关条约</text>
          </label>
        </view>
        <view class="btn">
          <button form-type="submit" @tap="submit">报名</button>
        </view>
        <view class="cancel text-center" @click="cancal">取消</view>
      </bjx-form>
    </view>
  </view>
</template>
