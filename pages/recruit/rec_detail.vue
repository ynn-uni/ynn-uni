<template>
  <view class="container rec_detail">
    <view class="title">{{data.title}}</view>
    <view class="cover">
      <image class="img" src="../../static/images/home_new.png" mode />
    </view>

    <view class="time">报名时间：{{data.start}}~{{data.end}}</view>
    <view class="info">
      <view class="info_item">年龄：{{data.age}}</view>
      <view class="info_item">性别：{{data.sex}}</view>
      <view class="info_item">其他：{{data.condition}}</view>
    </view>
    <view class="bref">
    ...
    <view class="btn_group">
      <button v-if="data.status" class="cu-btn" @tap="showModal">
        <view class="phone">
          <image src="../../static/icons/rec_phone.png" mode />资讯
        </view>
      </button>
      <button v-if="data.status" class="cu-btn" @click="handelApplication">
        <view class="phone rec">免费用药</view>
      </button>
      <button v-if="!data.status" class="cu-btn">
        <view class="phone end_phone">
          <image src="../../static/icons/end_phone.png" mode />资讯
        </view>
      </button>
      <button v-if="!data.status" class="cu-btn">
        <view class="phone end_rec">免费用药</view>
      </button>
    </view>
    <view class="cu-modal" :class="modalName=='Image'?'show':''">
      <view class="cu-dialog">
        <view class="tel">
          <view class>{{data.tel}}</view>
        </view>
    ...
    methods: {
      ...
      handelApplication() {
        if (!this.token) {
          ...
          uni.navigateTo({
            url:
              '/pages/recruit/rec_status?status=' + this.data.examine.data.status
          })
        }
      }
    }
